...
const OrderConfirmedPopup = ({ image, title, message, buttonLabel, orderId, onClose, embedded, paymentMethod, statusLabel, onTrack, trackLabel }: Props) => {
  ...
  const card = (
    <div className="relative bg-background w-full max-w-xs overflow-hidden shadow-2xl animate-scale-in border border-border">
      ...
      <div className="p-5 text-center space-y-3">
        ...
        {statusLabel && (
          <p className="text-[11px] inline-block px-2 py-1 border border-amber-500/40 text-amber-600 bg-amber-500/10 rounded">
            {statusLabel}
          </p>
        )}
        {onTrack && (
          <button onClick={onTrack} className="luxury-button-primary w-full text-[11px] py-2.5 mt-2">
