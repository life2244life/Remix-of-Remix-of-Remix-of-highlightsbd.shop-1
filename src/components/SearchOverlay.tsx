...
import { pushSearch } from '@/lib/gtm';
import { buildProductSearchOr } from '@/lib/searchUtils';
...
const SearchOverlay = ({ open, onClose, query, setQuery, onSubmit }: Props) => {
  ...
  const { data: suggestions = [], isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      const orExpr = buildProductSearchOr(debounced, ['name', 'brand', 'category', 'subcategory']);
      if (!orExpr) return [] as Product[];
  ...
  const goSearch = (term: string) => {
    saveRecent(term);
    pushSearch(term, suggestions.length);
