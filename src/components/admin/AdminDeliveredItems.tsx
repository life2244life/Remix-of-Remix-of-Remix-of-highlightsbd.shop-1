                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {Object.entries(sizes).sort(([a], [b]) => {
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 bg-[#22C5A0]/10 hover:bg-[#22C5A0]/25 transition-colors text-[#22C5A0] text-xs font-bold px-2 py-1 rounded cursor-pointer"
        ...
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-base">
              {selected?.product} — Size <span className="text-[#22C5A0]">{selected?.size}</span>
            </DialogTitle>
            <p className="text-xs text-muted-foreground">
              {matchingOrders.length} delivered order{matchingOrders.length !== 1 ? 's' : ''} •{' '}
                ...
                <div key={order.id} className="border border-border rounded-lg p-3 text-sm space-y-1">
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    ...
                    <div className="text-right">
                      <span className="inline-block bg-[#22C5A0]/10 text-[#22C5A0] text-xs font-bold px-2 py-0.5 rounded">
                        Qty: {qty}
                      </span>
