...
const AdminOfflineOrders = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Stock validation: block order if any item exceeds available stock
    const pids = Array.from(new Set(items.map(i => i.product_id).filter(Boolean))) as string[];
    if (pids.length) {
      const { data: stockRows } = await supabase.from('product_size_stock')
        .select('product_id,size,total_stock,sold_count,cancelled_count,returned_count')
        .in('product_id', pids);
      for (const it of items) {
        if (!it.product_id || !it.size) continue;
        const row: any = (stockRows || []).find((r: any) => r.product_id === it.product_id && r.size === it.size);
        const available = row ? (row.total_stock - row.sold_count + row.cancelled_count + row.returned_count) : 0;
        if (available <= 0) {
          toast.error(`Stock nei: ${it.name} (${it.size}) — order entry kora jabe na`);
          return;
        }
        if (available < it.quantity) {
          toast.error(`Only ${available} pcs available for ${it.name} (${it.size})`);
          return;
        }
      }
    }
    ...
    try {
      ...
      fireOrderEvent('offline_order_created', {
  ...
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={handleSubmit} className="border border-border p-6 space-y-6 bg-card">
        ...
        {/* City, Delivery Charge & Coupon */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-border">
          <div className="space-y-2">
            <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold flex items-center gap-1.5">
              <MapPin size={13} /> City & Delivery Charge
            </p>
            <input value={form.customer_city} onChange={e => setForm({ ...form, customer_city: e.target.value })} placeholder="City *" className="luxury-input text-xs" />
            <input type="number" value={form.delivery_charge} onChange={e => setForm({ ...form, delivery_charge: parseInt(e.target.value) || 0 })} placeholder="Delivery Charge (৳)" className="luxury-input text-xs" />
          </div>
      ...
      <OrderStatusList orders={offlineOrders} title="Offline Orders" onRefetch={refetch} />
    </div>
  );
};
