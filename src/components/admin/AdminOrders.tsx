...
import { useOrders, useUpdateOrder, useProducts, useDeliveryZones } from '@/hooks/useSupabase';
...
import { getPaymentMethodStyle, paymentMethodLabel, paymentMethodKey, PAYMENT_METHOD_FILTERS, getPaymentStatusStyle } from '@/lib/paymentMethods';
...
const AdminOrders = () => {
  ...
  const [editData, setEditData] = useState({
    ...
    customer_city: '', delivery_method: '', payment_method: '', delivery_charge: 0,
  });
  ...
  const filteredByPayment = paymentFilter === 'All'
    ? filteredByStatus
    : filteredByStatus.filter(o => paymentMethodKey((o as any).payment_method) === paymentFilter);
  const filtered = searchQuery.trim()
    ? filteredByPayment.filter(o => {
        ...
        return o.customer_name.toLowerCase().includes(q) ||
          ...
          paymentMethodLabel((o as any).payment_method).toLowerCase().includes(q);
      })
    : filteredByPayment;
  ...
  const handleStatusChange = async (id: string, newStatus: string, order: any) => {
    try {
      if (newStatus === 'Processing' && !order.consignment_id) {
        ...
        // COD orders: mark the pending COD payment as collected (success) on delivery.
        if (newStatus === 'Delivered' && order.payment_method === 'cod') {
          (supabase.rpc as any)('finalize_cod_payment', { _order_id: id })
            .then(({ error }) => { if (error) console.warn('[COD] finalize failed', error.message); });
        }
  ...
  const openOrder = (order: any) => {
    ...
    setEditData({
      ...
      delivery_charge: (order as any).delivery_charge || 0,
    });
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const discount = (selectedOrder as any).discount || 0;
      const newTotal = Math.max(0, subtotal + (editData.delivery_charge || 0) - discount);
      await updateOrder.mutateAsync({ id: selectedOrder.id, ...editData, total: newTotal });
      setSelectedOrder({ ...selectedOrder, ...editData, total: newTotal });
      setEditing(false);
  ...
  const exportOrders = () => {
    ...
    const exportData = toExport.map(o => ({
      ...
      payment_status_label: getPaymentStatusStyle((o as any).payment_status, o.payment_method).label,
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1.5 flex-wrap">
        ...
        <select
          ...
          className="text-[10px] tracking-[0.15em] uppercase px-3 py-2 border border-border bg-card text-muted-foreground hover:text-foreground hover:border-foreground/40 focus:outline-none cursor-pointer"
          title="Filter by payment method"
        >
          <option value="All">All Payments</option>
          {PAYMENT_METHOD_FILTERS.map(p => <option key={p.key} value={p.key}>{p.label}</option>)}
        </select>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  ...
                  <th className="text-left p-3 text-xs text-muted-foreground tracking-wider uppercase font-medium">Payment Method</th>
                  <th className="text-left p-3 text-xs text-muted-foreground tracking-wider uppercase font-medium">Payment</th>
              ...
              <tbody>
                {filtered.map(order => (
                  <tr key={order.id} className={`border-b border-border last:border-0 hover:bg-muted/20 transition-colors ${selectedIds.has(order.id) ? 'bg-primary/5' : ''}`}>
                    ...
                    <td className="p-3">
                      {(() => {
                        const pm = getPaymentMethodStyle((order as any).payment_method);
                        return pm ? (
                          <span className={`inline-flex items-center gap-1.5 text-[10px] font-medium px-2 py-1 rounded-sm whitespace-nowrap ${pm.badge}`}>
                    ...
                    <td className="p-3">
                      {(() => {
                        const ps = getPaymentStatusStyle((order as any).payment_status, (order as any).payment_method);
                        return (
                          <span className={`inline-flex items-center gap-1.5 text-[10px] font-medium px-2 py-1 rounded-sm whitespace-nowrap ${ps.badge}`}>
            ...
            <div className="space-y-3 text-sm">
              {editing ? (
                <>
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-muted-foreground tracking-wider uppercase">Delivery Zone</label>
                    {(() => {
                      const defaults = [
                        { name: 'Inside Dhaka', fee: 70 },
                        { name: 'Sub - Urban Dhaka', fee: 90 },
                        { name: 'Outside Dhaka', fee: 110 },
                      ];
                      const merged = defaults.map(d => {
                        const dbZone = deliveryZones.find(z => z.name === d.name);
                        return dbZone ? { name: dbZone.name, fee: dbZone.fee } : d;
                      });
                      deliveryZones.forEach(z => {
                        if (!merged.find(m => m.name === z.name)) merged.push({ name: z.name, fee: z.fee });
                      });
                      const currentInList = merged.find(m => m.name === editData.delivery_method);
                      return (
                        <select
                          value={editData.delivery_method}
                          onChange={e => {
                            const zone = merged.find(z => z.name === e.target.value);
                            setEditData(p => ({
                              ...p,
                              delivery_method: e.target.value,
                              delivery_charge: zone ? zone.fee : p.delivery_charge,
                            }));
                          }}
                          className="luxury-input w-full text-sm"
                        >
                          {!currentInList && editData.delivery_method && (
                            <option value={editData.delivery_method}>{editData.delivery_method}</option>
                          )}
                          {merged.map(z => (
                            <option key={z.name} value={z.name}>{z.name} — ৳{z.fee}</option>
                          ))}
                        </select>
                      );
                    })()}
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-muted-foreground tracking-wider uppercase">Delivery Charge</label>
                    <input
                      type="number"
                      value={editData.delivery_charge}
                      onChange={e => setEditData(p => ({ ...p, delivery_charge: parseInt(e.target.value) || 0 }))}
                      className="luxury-input w-full text-sm"
                    />
                  </div>
                  <div className="space-y-1">
                ...
                <>
                  ...
                  <p className="flex items-center gap-2">
                    ...
                    {(() => {
                      const pm = getPaymentMethodStyle(selectedOrder.payment_method);
                      return pm ? (
                        <span className={`inline-flex items-center gap-1.5 text-[10px] font-medium px-2 py-0.5 rounded-sm ${pm.badge}`}>
                          <span className="w-1.5 h-1.5 rounded-full" style={{ background: pm.dot }} />
                          {pm.label}
                        </span>
                      ) : <span>—</span>;
                    ...
                    {(() => {
                      const ps = getPaymentStatusStyle(selectedOrder.payment_status, selectedOrder.payment_method);
                      return (
                        <span className={`inline-flex items-center gap-1.5 text-[10px] font-medium px-2 py-0.5 rounded-sm ${ps.badge}`}>
                          <span className="w-1.5 h-1.5 rounded-full" style={{ background: ps.dot }} />
                          {ps.label}
                        </span>
                      );
              ...
              {(selectedOrder as any).coupon_code && (
                <p>
                  ...
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-primary/10 text-primary font-mono text-xs font-bold tracking-wider">
                    {(selectedOrder as any).coupon_code}
                  </span>
                  {((selectedOrder as any).discount || 0) > 0 && (
                    <span className="ml-2 text-muted-foreground">(-৳{((selectedOrder as any).discount).toLocaleString()})</span>
                  )}
                </p>
              )}
              ...
              {/* Payment transaction timeline & verification */}
              <OrderPaymentTimeline orderId={selectedOrder.id} />
                ...
                <button
                  onClick={() => {
                    ...
                    const msg = encodeURIComponent(
                      ...
                      `ধন্যবাদ আমাদের সাথে থাকার জন্য। — EIDLIP, Premium Men's Fashion`
                    );
