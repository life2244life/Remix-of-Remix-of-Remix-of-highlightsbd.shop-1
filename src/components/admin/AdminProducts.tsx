...
import { Edit, Trash2, Plus, Search, X, Upload, Image as ImageIcon, Copy, Check, ExternalLink } from 'lucide-react';
import { toast } from 'sonner';
import { supabase } from '@/integrations/supabase/client';
import { useQueryClient } from '@tanstack/react-query';
import { Switch } from '@/components/ui/switch';
import ProductInfoEditor from '@/components/admin/ProductInfoEditor';
import { getProductInfo, syncSpecificationWithSizes, type ProductInfoData } from '@/data/productInfo';
import SeoSidebar from '@/components/admin/SeoSidebar';
import { analyzeProductSEO } from '@/lib/seoAnalyzer';
import { createPortal } from 'react-dom';
import { useAuth } from '@/contexts/AuthContext';
import { useCategoryTree } from '@/hooks/useCategoryTree';
import { useFormDraft } from '@/hooks/useFormDraft';
import { DraftRecoveryBanner, ClearAllDialog } from '@/components/admin/EditorDraftUI';
import TextFieldWithGuidance, { FieldCounter } from '@/components/admin/TextFieldWithGuidance';
...
const AdminProducts = () => {
  ...
  const openEditor = (p: Product) => {
    setEditingProduct({
      ...
      size_chart: Array.isArray(p.size_chart) ? p.size_chart : [],
    } as Product);
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => { setShowAddForm(true); setEditingProduct({ id: '', name: '', price: 0, original_price: null, image_url: '', category: 'T-Shirt', description: '', sizes: ['S', 'M', 'L', 'XL'], colors: [{ name: 'Black', hex: '#1a1a1a' }], stock: 0, featured: false, is_new_drop: false, is_active: true, brand: 'EIDLIP', sku: '', size_chart: [], created_at: '', updated_at: '' } as any); }} className="inline-flex items-center gap-2 px-4 py-2.5 text-[10px] uppercase tracking-widest bg-foreground text-background hover:opacity-90 transition-opacity">
      ...
      {editingProduct && (
        <ProductForm product={editingProduct} isNew={showAddForm} existingSkus={products.map((p: any) => p.sku).filter(Boolean)} onSave={async (p) => {
          try {
            if (showAddForm) {
                ...
                <div className="relative aspect-[3/4] overflow-hidden bg-gradient-to-br from-muted/40 to-muted/10">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-foreground/70 via-foreground/0 to-foreground/0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center gap-2 pb-3 pointer-events-none">
                    <a
                      ...
                      className="pointer-events-auto p-2 bg-background text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <a href={`/products/${(p as any).slug || p.id}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-2 text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" title="View on website"><ExternalLink size={13} /></a>
                  <button onClick={(e) => { e.stopPropagation(); openEditor(p); }} className="p-2 hover:bg-muted transition-colors"><Edit size={13} /></button>
...
const GalleryUpload = ({ gallery, onChange, colorOptions = [], imageColors = {}, onColorChange }: {
  gallery: string[];
    ...
    <div className="space-y-3">
      ...
      >
        {uploading ? (
          ...
          <>
            ...
            <p className="text-[10px] text-muted-foreground mt-1">JPG · PNG · WEBP · max 100MB · select many at once</p>
          </>
        )}
      </div>
      ...
      {gallery.length > 0 && (
        <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
          {gallery.map((url, idx) => (
            <div key={`${url}-${idx}`} className="space-y-1">
              <div className="relative group border border-border bg-muted/20 aspect-[3/4] overflow-hidden">
              ...
              {idx !== 0 && colorOptions.length > 0 && onColorChange && (
                <select
                  value={imageColors[url] || ''}
                  onChange={(e) => onColorChange(url, e.target.value)}
                  className="w-full bg-transparent border border-border text-[10px] px-1 py-1 outline-none focus:border-foreground/40"
                  title="Color variant this image belongs to"
                >
...
const ProductForm = ({ product, isNew, existingSkus = [], onSave, onCancel, onDone }: { product: Product; isNew: boolean; existingSkus?: string[]; onSave: (p: Product) => Promise<any>; onCancel: () => void; onDone: () => void }) => {
  ...
  const safeProduct = {
    ...
    alt_text: (product as any).alt_text || '',
  ...
  const [form, setForm] = useState({
    ...
    product_info: getProductInfo((product as any).product_info),
  });
  const [slugError, setSlugError] = useState('');
  const slugify = (s: string) => s.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
  // Auto SKU only applies to NEW products and stops once the admin edits it.
  const [skuTouched, setSkuTouched] = useState(false);

  const [savedProductId, setSavedProductId] = useState(isNew ? '' : product.id);
  const [sizeStocks, setSizeStocks] = useState<Record<string, number>>({});
  const [existingSizeStocks, setExistingSizeStocks] = useState<Record<string, { id: string; total_stock: number; sold_count: number; cancelled_count: number; returned_count: number }>>({});
  const [gallery, setGallery] = useState<string[]>(product.image_url ? [product.image_url] : []);
  const [originalExtras, setOriginalExtras] = useState<Array<{ id: string; image_url: string; color: string | null }>>([]);
  const [galleryColors, setGalleryColors] = useState<Record<string, string>>({});
  const [saving, setSaving] = useState(false);
  const qc = useQueryClient();
  const [focusKeyword, setFocusKeyword] = useState('');

  // Live category tree (realtime — new categories/sub-categories appear instantly).
  const { categories: catTree, subcategories } = useCategoryTree();
  const categoryNames = useMemo(() => {
    const names = catTree.map(c => c.name);
    if (form.category && !names.includes(form.category)) names.unshift(form.category);
    return names;
  }, [catTree, form.category]);

  // Auto-generate the SKU for new products (e.g. EID-TS-006) based on category.
  // Regenerates when the category changes, until the admin manually edits it.
  useEffect(() => {
    if (!isNew || skuTouched) return;
    const next = generateSku(form.category, existingSkus);
    setForm(f => (f.sku === next ? f : { ...f, sku: next }));
  }, [isNew, skuTouched, form.category, existingSkus]);

  // ---- Auto-draft / recovery ----
  const { user } = useAuth();
  const userId = user?.uid || 'anon';
  const draftKey = userId
    ? (isNew ? `product_draft_${userId}` : `product_draft_${userId}_edit_${product.id}`)
    : null;
  const draftSnapshot = useMemo(
    () => ({ form, gallery, galleryColors, sizeStocks, focusKeyword }),
    [form, gallery, galleryColors, sizeStocks, focusKeyword],
  );
  const { recovered, clearDraft, dismissRecovery } = useFormDraft(draftKey, draftSnapshot);
  const [clearOpen, setClearOpen] = useState(false);

  const applyRecovered = () => {
    if (!recovered) return;
    if (recovered.form) setForm(recovered.form);
    if (recovered.gallery) setGallery(recovered.gallery);
    if (recovered.galleryColors) setGalleryColors(recovered.galleryColors);
    if (recovered.sizeStocks) setSizeStocks(recovered.sizeStocks);
    if (typeof recovered.focusKeyword === 'string') setFocusKeyword(recovered.focusKeyword);
    dismissRecovery();
  };
  ...
  const handleClearAll = () => {
    setForm({
      ...(form as any),
      name: '', brand: 'EIDLIP', sku: '', description: '',
      price: 0, original_price: null, image_url: '',
      sizes: [], colors: [], stock: 0, featured: false, is_new_drop: false,
      subcategory: null, slug: '', alt_text: '', seo_title: '', seo_description: '',
      og_image: '', noindex: false, size_chart: [],
      product_info: getProductInfo(undefined),
    });
    setGallery([]);
    setGalleryColors({});
    setSizeStocks({});
  ...
  const seoResult = useMemo(() => analyzeProductSEO({
    focusKeyword,
    name: form.name,
    ...
    imageCount: gallery.filter(Boolean).length,
    structuredData: true,
    canonical: true,
  }), [focusKeyword, form.name, form.slug, form.description, form.seo_title, form.seo_description, form.alt_text, gallery]);

  // Load existing per-size stock + extra images for this product
  useEffect(() => {
    const pid = savedProductId || (!isNew ? product.id : '');
    if (!pid) return;
    supabase.from('product_size_stock').select('id, size, total_stock, sold_count, cancelled_count, returned_count').eq('product_id', pid).then(({ data }) => {
      const ex: Record<string, { id: string; total_stock: number; sold_count: number; cancelled_count: number; returned_count: number }> = {};
      const stocks: Record<string, number> = {};
      (data || []).forEach((r: any) => {
        ex[r.size] = { id: r.id, total_stock: r.total_stock, sold_count: r.sold_count || 0, cancelled_count: r.cancelled_count || 0, returned_count: r.returned_count || 0 };
        // Show AVAILABLE units in the form (what the customer sees)
        stocks[r.size] = Math.max(0, r.total_stock - (r.sold_count || 0) + (r.cancelled_count || 0) + (r.returned_count || 0));
      });
      setExistingSizeStocks(ex);
      setSizeStocks(stocks);
    });
    supabase.from('product_images').select('id, image_url, sort_order, color').eq('product_id', pid).order('sort_order').then(({ data }) => {
      const extras = (data || []) as any[];
      setOriginalExtras(extras.map(e => ({ id: e.id, image_url: e.image_url, color: e.color ?? null })));
      const colorMap: Record<string, string> = {};
      extras.forEach(e => { if (e.color) colorMap[e.image_url] = e.color; });
      setGalleryColors(colorMap);
      setGallery(g => {
  ...
  // Products store the category NAME (e.g. "Men") while sub-categories reference the
  // parent category SLUG (e.g. "men") — map name → slug so the right sub-categories show.
  const selectedCategorySlug = catTree.find(c => c.name === form.category)?.slug || form.category;
  const availableSubs = subcategories.filter(
    s => s.parent_category === selectedCategorySlug || s.parent_category === form.category,
  );
  const formSizes = Array.isArray(form.sizes) ? form.sizes : [];

  const persistSizeStocks = async (productId: string) => {
    for (const size of formSizes) {
      const availQty = sizeStocks[size] ?? 0;
      const ex = existingSizeStocks[size];
      if (ex) {
        // Convert desired AVAILABLE units back into cumulative total_stock
        const newTotal = availQty + ex.sold_count - ex.cancelled_count - ex.returned_count;
        const prevAvail = Math.max(0, ex.total_stock - ex.sold_count + ex.cancelled_count + ex.returned_count);
        if (ex.total_stock === newTotal) continue;
        await supabase.from('product_size_stock').update({ total_stock: newTotal } as any).eq('id', ex.id);
        await supabase.from('stock_logs').insert({
          product_id: productId, size, change_type: 'manual',
          quantity: availQty - prevAvail, notes: `Available set to ${availQty} (from product form)`,
        });
      } else {
        await supabase.from('product_size_stock').insert({ product_id: productId, size, total_stock: availQty });
        await supabase.from('stock_logs').insert({
          product_id: productId, size, change_type: 'manual',
          quantity: availQty, notes: `Initial stock ${availQty} (from product form)`,
        });
      }
    }
  ...
  const persistGallery = async (productId: string) => {
    ...
    for (let i = 0; i < extras.length; i++) {
      const url = extras[i];
      const color = galleryColors[url] || null;
      const existing = originalExtras.find(o => o.image_url === url);
      if (!existing) {
        await supabase.from('product_images').insert({ product_id: productId, image_url: url, sort_order: i, color } as any);
      } else if ((existing.color ?? null) !== color) {
        // color (or order) changed for an existing image
        await supabase.from('product_images').update({ color, sort_order: i } as any).eq('id', existing.id);
      }
    }
    ...
    try {
      ...
      if (finalSlug) {
        const { data: dup } = await supabase
          ...
          .select('id')
          .eq('slug', finalSlug)
          .neq('id', savedProductId || product.id || '00000000-0000-0000-0000-000000000000')
          .maybeSingle();
        if (dup) {
          setSlugError(`Slug "${finalSlug}" is already used`);
          toast.error(`Slug "${finalSlug}" is already used by another product`);
          setSaving(false);
          return;
        }
      }
      ...
      const payload = {
        ...form,
        slug: finalSlug,
        image_url: gallery[0] || form.image_url || '',
        product_info: {
          ...(form.product_info as ProductInfoData),
          specification: syncSpecificationWithSizes((form.product_info as ProductInfoData).specification, formSizes),
        },
      };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onCancel}>
      ...
      <div
        className="relative w-full max-w-5xl max-h-[90vh] bg-background border border-border shadow-2xl overflow-y-auto animate-in fade-in zoom-in-95 duration-200"
        onClick={e => e.stopPropagation()}
      >
        {/* Sticky header */}
        ...
        <div className="p-6 flex flex-col lg:flex-row gap-6">
          ...
          <SeoSidebar
            ...
            keywordPlaceholder="e.g. Men's Panjabi"
            preview={{
              title: form.seo_title || form.name,
              description: form.seo_description || form.description,
              path: `/products/${form.slug || slugify(form.name) || 'product-slug'}`,
              image: gallery.find(Boolean) || form.image_url || undefined,
            }}
          />
          ...
          <Section
            ...
            subtitle="Product identity and category"
            action={
              <button
                type="button"
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <TextFieldWithGuidance
                label="SKU"
                value={form.sku}
                onChange={v => { setSkuTouched(true); setForm({ ...form, sku: v }); }}
                placeholder="EID-TS-001"
                recommendedMin={3}
                recommendedMax={32}
                mono
                hint={isNew ? <p className="text-[10px] text-muted-foreground mt-1">Auto-generated from category — edit if needed.</p> : undefined}
              />
            ...
            <div className="flex flex-wrap gap-2">
              {['XS', 'S', 'M', 'L', 'XL', 'XXL', '2XL', '3XL'].map(size => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    const sizes = formSizes.includes(size)
                      ? formSizes.filter(s => s !== size)
                      : [...formSizes, size];
                    setForm({ ...form, sizes });
                  }}
                  className={`min-w-[44px] px-3 py-2 text-xs uppercase tracking-widest border transition-all ${formSizes.includes(size) ? 'bg-foreground text-background border-foreground' : 'border-border text-muted-foreground hover:border-foreground/40 hover:text-foreground'}`}
                >
                  {size}
                </button>
              ))}
          ...
          {/* Section: Images */}
          <Section title="Images" subtitle="First image becomes the main · tag each extra image with a color variant">
            <GalleryUpload
              gallery={gallery}
              onChange={setGallery}
              colorOptions={(Array.isArray(form.colors) ? form.colors : []).map((c: any) => c?.name).filter(Boolean)}
              imageColors={galleryColors}
              onColorChange={(url, color) => setGalleryColors(prev => ({ ...prev, [url]: color }))}
            />
          ...
          <Section title="Description" subtitle="Product details shown to customers">
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-3 border border-border bg-muted/20 px-3 py-2.5">
                <div>
                  <p className="text-[11px] uppercase tracking-widest font-medium">Description</p>
                  <p className="text-[10px] text-muted-foreground mt-0.5">Turn this product description on/off</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-[9px] uppercase tracking-widest ${(form.product_info as ProductInfoData).description.enabled ? 'text-emerald-600 dark:text-emerald-400' : 'text-muted-foreground'}`}>
                    {(form.product_info as ProductInfoData).description.enabled ? 'On' : 'Off'}
                  </span>
                  <Switch
                    checked={(form.product_info as ProductInfoData).description.enabled}
                    onCheckedChange={(enabled) => setForm({
                      ...form,
                      product_info: { ...(form.product_info as ProductInfoData), description: { enabled } },
                    })}
                  />
                </div>
              </div>
              {(form.product_info as ProductInfoData).description.enabled && (
                <TextFieldWithGuidance
          ...
          {/* Section: Product Information (managed sections + toggles) */}
          <Section title="Product Information Sections" subtitle="Turn each section on/off and manage its content. Off sections are hidden on the product page.">
            <ProductInfoEditor
              value={form.product_info as ProductInfoData}
              sizes={formSizes}
              onSizesChange={(sizes) => setForm({ ...form, sizes })}
              onChange={(info) => setForm({ ...form, product_info: info })}
            />
          </Section>
          ...
          {/* Section: SEO */}
          <Section title="SEO & Search Visibility" subtitle="Control how this product appears on Google & social media">
            <div className="space-y-4">
              <TextFieldWithGuidance
                label="URL Slug (auto from name if empty)"
                value={form.slug}
                onChange={v => { setForm({ ...form, slug: v }); setSlugError(''); }}
                ...
                hint={
                  <>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">URL: /products/{form.slug || slugify(form.name) || 'product-slug'}</p>
                  </>
                }
              />
              <TextFieldWithGuidance
                ...
                placeholder={form.name || 'Describe the product image'}
                recommendedMin={80}
                recommendedMax={125}
              />
              <TextFieldWithGuidance
                ...
                onChange={v => setForm({ ...form, seo_title: v.slice(0, 70) })}
              ...
              <TextFieldWithGuidance
                ...
                onChange={v => setForm({ ...form, seo_description: v.slice(0, 170) })}
                placeholder={form.description.slice(0, 155) || 'Meta description'}
                recommendedMin={140}
                recommendedMax={160}
                maxLength={170}
                multiline
              />
              <Field label="Open Graph Image (1200x630 recommended)">
                <input value={form.og_image} onChange={e => setForm({ ...form, og_image: e.target.value })} placeholder="https://... (defaults to main product image)" className="luxury-input text-xs" />
              </Field>
              <label className={`flex items-center gap-2 px-4 py-2.5 border cursor-pointer transition-colors ${form.noindex ? 'border-destructive/40 bg-destructive/5' : 'border-border hover:border-foreground/30'}`}>
                <input type="checkbox" checked={form.noindex} onChange={e => setForm({ ...form, noindex: e.target.checked })} />
                <span className="text-xs uppercase tracking-widest">No-index (hide from Google)</span>
              </label>
            </div>
          </Section>
          </div>
      ...
      <ClearAllDialog
        ...
        items={['Text', 'Images', 'SEO data', 'Categories', 'Specifications', 'FAQs', 'Trust badges', 'Draft data']}
      />
    </div>,
    document.body,
  );
