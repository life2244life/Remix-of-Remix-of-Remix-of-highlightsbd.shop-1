...
type StockDisplayRow = { key: string; size: string; available: number; stock?: SizeStock };

const getRowAvailable = (row: StockDisplayRow) => row.available;
const getRowKey = (row: StockDisplayRow) => row.key;

const deriveProductSizes = (product: any): string[] => {
  const directSizes = (Array.isArray(product?.sizes) ? product.sizes : [])
    .map((size: unknown) => safeSize(size).trim())
    .filter(Boolean);
  if (directSizes.length > 0) return directSizes;

  const chart = Array.isArray(product?.size_chart) ? product.size_chart : [];
  return chart
    .map((row: any) => row?.Size ?? row?.size ?? row?.SIZE ?? Object.values(row || {})[0])
    .map((size: unknown) => safeSize(size).trim())
    .filter(Boolean);
};

const resolveStockRows = (product: any, stocks: SizeStock[], includeMissingSizes = false): StockDisplayRow[] => {
  const productSizes = deriveProductSizes(product);
  const unusedStocks = [...stocks];
  const takeStockForSize = (size: string) => {
    const matchIndex = unusedStocks.findIndex(stock => normalizeSize(stock.size) === normalizeSize(size));
    const fallbackIndex = matchIndex >= 0 ? matchIndex : unusedStocks.findIndex(stock => !safeSize(stock.size).trim());
    return fallbackIndex >= 0 ? unusedStocks.splice(fallbackIndex, 1)[0] : undefined;
  };
  const rows = productSizes.flatMap((size): StockDisplayRow[] => {
    const stock = takeStockForSize(size);
    if (!stock && !includeMissingSizes) return [];
    return [{ key: stock?.id || `missing-${product?.id}-${size}`, size, available: stock ? getAvailable(stock) : 0, stock }];
  });
  unusedStocks.forEach((stock, index) => {
    const rawSize = safeSize(stock.size).trim();
    rows.push({ key: stock.id, size: rawSize || `Size ${index + 1}`, available: getAvailable(stock), stock });
  });
  return rows;
};
...
const AdminStockManagement = () => {
  ...
  const summary = useMemo(() => {
    ...
    products.forEach((p: any) => {
      ...
      const rows = resolveStockRows(p, stocks, true);
      const total = rows.reduce((sum, row) => sum + getRowAvailable(row), 0);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {filteredProducts.map((p: any) => {
          const stocks = stockByProduct[p.id] || [];
          const stockRows = resolveStockRows(p, stocks, true);
          const total = stockRows.reduce((sum, row) => sum + getRowAvailable(row), 0);
          const isOut = total <= 0 && stockRows.length > 0;
            ...
            >
              ...
              <div className="flex flex-wrap gap-1.5 mt-4">
                {stockRows.length === 0 ? (
                  ...
                  stockRows.map(row => {
                    const avail = getRowAvailable(row);
                    const sizeLabel = row.size;
                    return (
                      <span
                        key={getRowKey(row)}
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-semibold tracking-wider uppercase border rounded-full ${pillTone(avail)}`}
                        title={`Size ${sizeLabel}: ${avail} available`}
                      >
                        <span>{sizeLabel}</span>
...
const safeSize = (s: unknown) => (s == null ? '' : String(s));
const normalizeSize = (s: unknown) => safeSize(s).trim().toUpperCase();
const findStock = (stocks: SizeStock[], size: unknown) =>
  stocks.find(s => normalizeSize(s.size) === normalizeSize(size));

const StockEditDrawer = ({ product, stocks, onClose, onSaved }: { product: any; stocks: SizeStock[]; onClose: () => void; onSaved: () => void; }) => {
  const productSizes: string[] = deriveProductSizes(product);
  const stockRowsForEdit = resolveStockRows(product, stocks, true);
  const allSizes: string[] = stockRowsForEdit.map(row => row.size);
  const stockByDisplaySize = new Map<string, SizeStock>();
  stockRowsForEdit.forEach(row => {
    if (row.stock) stockByDisplaySize.set(normalizeSize(row.size), row.stock);
  });

  // Edit field represents AVAILABLE units (what customer sees), not cumulative total_stock.
  const availableOf = (ex: SizeStock | undefined) =>
    ex ? ex.total_stock - ex.sold_count + ex.cancelled_count + ex.returned_count : 0;

  const initial: Record<string, number> = {};
  allSizes.forEach((size: string) => {
    initial[size] = Math.max(0, availableOf(stockByDisplaySize.get(normalizeSize(size))));
  });
  ...
  const save = async () => {
    setSaving(true);
    try {
      for (const [size, availQty] of Object.entries(edit)) {
        const existing = stockByDisplaySize.get(normalizeSize(size));
        if (existing) {
          // Solve: availQty = newTotal - sold + cancelled + returned
          const newTotal = availQty + existing.sold_count - existing.cancelled_count - existing.returned_count;
          const sizeChanged = normalizeSize(existing.size) !== normalizeSize(size);
          if (existing.total_stock === newTotal && !sizeChanged) continue;
          await supabase.from('product_size_stock').update({ total_stock: newTotal, size } as any).eq('id', existing.id);
          await supabase.from('stock_logs').insert({
            product_id: product.id, size, change_type: 'manual',
            quantity: availQty - availableOf(existing),
            notes: `Available set to ${availQty}`,
          });
        } else {
          await supabase.from('product_size_stock').insert({ product_id: product.id, size, total_stock: availQty });
          await supabase.from('stock_logs').insert({
            product_id: product.id, size, change_type: 'manual',
            quantity: availQty, notes: `Initial stock ${availQty}`,
          });
        }
      }
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="space-y-3">
            {allSizes.map((size: string) => {
              const existing = stockByDisplaySize.get(normalizeSize(size));
              const planned = edit[size] ?? availableOf(existing);
              const avail = planned;
              const isOrphan = !productSizes.some(ps => normalizeSize(ps) === normalizeSize(size));
              const label = safeSize(size).trim() === '' ? '(no size)' : safeSize(size);

              return (
                <div key={size || '__empty__'} className={`border p-4 space-y-3 ${isOrphan ? 'border-amber-500/40 bg-amber-500/[0.03]' : 'border-border'}`}>
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span className="text-base font-medium tracking-wider">{label}</span>
                      {isOrphan && (
                        <span className="text-[9px] uppercase tracking-widest text-amber-600 dark:text-amber-400 border border-amber-500/40 px-1.5 py-0.5 rounded-full">
                          orphan
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {isOrphan && existing && (
                        <button
                          type="button"
                          onClick={async () => {
                            if (!confirm(`Delete orphan stock row "${label}" (${existing.total_stock} units)?`)) return;
                            await supabase.from('product_size_stock').delete().eq('id', existing.id);
                            toast.success('Orphan row deleted');
                            onSaved();
                          }}
                          className="p-1.5 text-destructive border border-destructive/30 hover:bg-destructive/10"
                          title="Delete this orphan row"
                        ><X size={12} /></button>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => setEdit(p => ({ ...p, [size]: Math.max(0, (p[size] ?? availableOf(existing)) - 1) }))}
                    ...
                    <button
                      type="button"
                      onClick={() => setEdit(p => ({ ...p, [size]: (p[size] ?? availableOf(existing)) + 1 }))}
                      className="w-10 h-10 border border-border hover:bg-muted flex items-center justify-center transition-colors"
                    ><Plus size={14} /></button>
                  </div>
