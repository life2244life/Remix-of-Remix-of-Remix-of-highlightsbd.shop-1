...
import { RecProduct, discountPct, formatPrice, labelClasses } from "./mockData";
    ...
    >
      ...
      >
        <img
          src={product.image}
          alt={product.name}
          className={`aspect-[3/4] w-full object-cover transition-all duration-500 group-hover:scale-105 ${hovered ? "opacity-0" : "opacity-100"}`}
          loading="lazy"
        />
        <img
        ...
        {product.label && (
          <motion.span
            ...
            className={`absolute left-3 top-3 rounded-[8px] px-2 py-0.5 text-[10px] font-bold tracking-wide ${labelClasses[product.label]}`}
          >
            {product.label}
          </motion.span>
        )}
        ...
        <button
          ...
          className="absolute bottom-3 right-3 flex h-9 w-9 items-center justify-center rounded-full bg-background/80 text-foreground opacity-0 backdrop-blur transition-all duration-300 hover:bg-background group-hover:opacity-100"
        >
