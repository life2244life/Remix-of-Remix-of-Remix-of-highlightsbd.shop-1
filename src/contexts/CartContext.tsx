...
import { pushAddToCart, pushRemoveFromCart } from '@/lib/gtm';
...
declare global {
  var __eidlipCartContext: React.Context<CartContextType | undefined> | undefined;
}

const CartContext = globalThis.__eidlipCartContext ?? createContext<CartContextType | undefined>(undefined);
globalThis.__eidlipCartContext = CartContext;

export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [items, setItems] = useState<CartItem[]>(() => {
    const saved = localStorage.getItem('eidlip-cart');
  ...
  useEffect(() => {
    localStorage.setItem('eidlip-cart', JSON.stringify(items));
  }, [items]);
  ...
  const removeItem = useCallback((productId: string, size: string, color: string) => {
    setItems(prev => {
      const removed = prev.find(i => i.product.id === productId && i.size === size && i.color === color);
      if (removed) pushRemoveFromCart(removed.product, removed.quantity, removed.size, removed.color);
      return prev.filter(i => !(i.product.id === productId && i.size === size && i.color === color));
    });
  }, []);
