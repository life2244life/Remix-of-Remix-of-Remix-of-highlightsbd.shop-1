 ...
 */
const compressImage = (file: File, maxWidth = 7680, quality = 0.95): Promise<Blob> => {
...
export const uploadImage = async (file: File, folder: string = 'products'): Promise<string> => {
  ...
  const maxSize = 100 * 1024 * 1024; // 100MB to accommodate 8K images
  if (file.size > maxSize) throw new Error('Image must be less than 100MB');
