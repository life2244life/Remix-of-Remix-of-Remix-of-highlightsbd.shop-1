...
import AdminLinkInspector from '@/components/admin/AdminLinkInspector';
...
const Admin = () => {
  ...
  const setActiveTab = (next: string) => navigate(`/admin/${next}`);
  ...
  return (
    <SidebarProvider>
      <SEO title="Admin Panel" description="EIDLIP admin dashboard." path="/admin" noIndex />
      <div className="min-h-screen flex w-full bg-background">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-auto">
            ...
            {activeTab === 'homepage-sections' && <AdminHomepageSections />}
            {activeTab === 'product-sections' && <AdminProductSections />}
            {activeTab === 'website-settings' && <AdminWebsiteSettings />}
            {activeTab === 'templates' && <AdminTemplates />}
            {activeTab === 'subcategories' && <AdminSubcategories />}
            {activeTab === 'custom-pages' && <AdminContent />}
            {activeTab === 'popups' && <AdminPopups />}
            {activeTab === 'blog' && <AdminBlog />}
            {activeTab === 'products' && <AdminProducts />}
            {activeTab === 'collections' && <AdminCollections />}
            {activeTab === 'seo-settings' && <AdminSEOSettings />}
            {activeTab === 'link-inspector' && <AdminLinkInspector />}
