...
import { useActivePaymentMethods, useCodDistrictRules, matchCodRule } from '@/hooks/usePayments';
import { initSslcommerz, createBkash, initNagad, REDIRECT_GATEWAYS, uploadPaymentProof, submitManualPayment, getManualReceiver } from '@/lib/payments';
import OrderConfirmedPopup from '@/components/OrderConfirmedPopup';
import SEO from '@/components/SEO';
import { supabase } from '@/integrations/supabase/client';
import { sendOrderEmail } from '@/lib/api';
import { toast } from 'sonner';
import { Check, Copy, ChevronDown, Upload } from 'lucide-react';
...
type PaymentMethod = 'cod' | 'online' | 'automated' | 'manual';
...
const Checkout = () => {
  ...
  const { data: codRules = [] } = useCodDistrictRules();
  ...
  const [automatedGateway, setAutomatedGateway] = useState<string | null>(null);
  const [selectedManualGateway, setSelectedManualGateway] = useState<string | null>(null);
  ...
  // couponDiscount is derived from current cart total + appliedCoupon (see useMemo below)
  const [showConfirmedPopup, setShowConfirmedPopup] = useState(false);
  const [confirmedOrderId, setConfirmedOrderId] = useState<string>('');
  const [pendingVerification, setPendingVerification] = useState(false);
  const [confirmedMethodName, setConfirmedMethodName] = useState<string>('');
  ...
  // ----- Payment business rules (DB-driven via store_settings) -----
  const minOrderAmount = parseInt(storeSettings?.payment_min_order_amount ?? '0') || 0;
  const freeShipThreshold = parseInt(storeSettings?.payment_free_shipping_threshold ?? '0') || 0;
  const advanceEnabled = (storeSettings?.payment_advance_enabled ?? 'false') === 'true';
  const advancePercent = Math.min(100, Math.max(0, parseInt(storeSettings?.payment_advance_percent ?? '0') || 0));

  const couponFreeShipping = appliedCoupon?.discount_type === 'free_shipping';
  const thresholdFreeShipping = freeShipThreshold > 0 && total >= freeShipThreshold;
  const isFreeShipping = couponFreeShipping || thresholdFreeShipping;
  ...
  // ----- COD district rule + advance payment derivation -----
  const codGloballyActive = activeMethods.some(m => m.gateway === 'cod');
  const codRule = useMemo(() => matchCodRule(codRules, form.city), [codRules, form.city]);
  const codAllowed = codGloballyActive && (codRule ? codRule.cod_allowed : true);
  const advanceForCod = useMemo(() => {
    if (!codRule?.advance_required) {
      return advanceEnabled ? Math.round(grandTotal * advancePercent / 100) : 0;
    }
    if (codRule.advance_amount && codRule.advance_amount > 0) return Math.min(codRule.advance_amount, grandTotal);
    return advanceEnabled ? Math.round(grandTotal * advancePercent / 100) : 0;
  }, [codRule, advanceEnabled, advancePercent, grandTotal]);
  const advanceRequired = !!codRule?.advance_required && advanceForCod > 0;

  // Automated gateways available at checkout (exclude COD which has its own card)
  const automatedMethods = useMemo(
    () => activeMethods.filter(m => (REDIRECT_GATEWAYS as readonly string[]).includes(m.gateway)),
    [activeMethods]
  );

  // Manual gateways (bKash/Nagad/Rocket/Upay/Bank Transfer, etc.) — DB-driven.
  const manualMethods = useMemo(
    () => activeMethods.filter(m => m.gateway_type === 'manual'),
    [activeMethods]
  );

  // If COD becomes unavailable for the area, fall back to an online gateway
  useEffect(() => {
    if (payment === 'cod' && !codAllowed && automatedMethods.length > 0) {
      setPayment('automated');
      setAutomatedGateway(automatedMethods[0].gateway);
    }
  }, [codAllowed, automatedMethods, payment]);

  // Auto-remove coupon if cart total drops below min_order_amount
  useEffect(() => {
    if (appliedCoupon?.min_order_amount && total < appliedCoupon.min_order_amount) {
      toast.error(`Coupon removed: minimum order ৳${appliedCoupon.min_order_amount} required`);
  ...
  const startGatewayRedirect = async (gw: string, orderId: string, amount: number, isAdvance: boolean) => {
    let res;
    if (gw === 'sslcommerz') res = await initSslcommerz(orderId, amount, isAdvance);
    else if (gw === 'bkash') res = await createBkash(orderId, amount, isAdvance);
    else if (gw === 'nagad') res = await initNagad(orderId, amount, isAdvance);
    else throw new Error('Unsupported payment gateway');
    if (res?.error || !res?.url) throw new Error(res?.error || 'Failed to start payment');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (payment === 'cod' && !codAllowed) {
      toast.error('Cash on Delivery is not available for your area');
      return;
    }
    if (payment === 'automated' && !automatedGateway) {
      toast.error('Please select a payment method');
      return;
    }
    if (payment === 'manual') {
      if (!selectedManualGateway) {
        toast.error('Please select a payment method');
        return;
      }
      const m = manualMethods.find(x => x.gateway === selectedManualGateway);
      const r = getManualReceiver(m?.manual_config);
      if (!form.senderNumber) {
        toast.error('Please enter sender number');
        return;
      }
      if (r.requireTxnId && !form.transactionId) {
        toast.error('Please enter the transaction ID');
        return;
      }
      if (r.requireScreenshot && !proofFile) {
    ...
    try {
      const orderResult = await createOrder.mutateAsync({
        ...
        payment_method: payment === 'automated' ? automatedGateway!
          : payment === 'manual' ? selectedManualGateway!
          : (payment === 'online' ? onlineProvider! : 'cod'),
      ...
      // Coupon usage is incremented atomically server-side inside create_order RPC.
      clearCart();
      try { localStorage.removeItem('spin_wheel_spun_v1'); } catch {}

      // Manual payment: upload screenshot (if any) + record a 'submitted' transaction
      if (payment === 'manual' && selectedManualGateway && orderResult?.id) {
        try {
          let proofPath: string | null = null;
          if (proofFile) proofPath = await uploadPaymentProof(proofFile, orderResult.id);
          await submitManualPayment({
            order_id: orderResult.id,
            gateway: selectedManualGateway,
            sender_number: form.senderNumber || null,
            transaction_id: form.transactionId || null,
            proof_url: proofPath,
          });
        } catch (mErr: any) {
          toast.error(mErr.message || 'Payment submission failed. Our team will follow up.');
        }
      }

      // Online gateways / advance payment: redirect to the gateway to complete payment
      const advGw = automatedGateway || automatedMethods[0]?.gateway;
      const needsRedirect = payment === 'automated' || (payment === 'cod' && advanceRequired && !!advGw);
      if (needsRedirect && orderResult?.id) {
        setRedirecting(true);
        try {
          const gw = payment === 'automated' ? automatedGateway! : advGw!;
          const amount = payment === 'automated' ? grandTotal : advanceForCod;
          await startGatewayRedirect(gw, orderResult.id, amount, payment === 'cod');
          return;
        } catch (gwErr: any) {
          setRedirecting(false);
          toast.error(gwErr.message || 'Could not start online payment');
          navigate(`/checkout/complete?order=${orderResult.id}&status=failed`);
          return;
        }
      }

      const isManualPending = payment === 'manual' && !!selectedManualGateway;
      if (isManualPending) {
        const m = manualMethods.find(x => x.gateway === selectedManualGateway);
        setConfirmedMethodName(m?.display_name || selectedManualGateway || 'Manual Payment');
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO title="Checkout" description="Complete your EIDLIP order — secure checkout with cash on delivery available." path="/checkout" noIndex />
      <Header />
      <CartDrawer />
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              ...
              <div className="border border-border p-5 mt-6">
                ...
                {minOrderAmount > 0 && total < minOrderAmount && (
                  <p className="text-xs text-destructive mb-3">Minimum order amount is ৳{minOrderAmount.toLocaleString()} to place an order.</p>
                )}
                <div className="space-y-3">
                  {/* Cash on Delivery */}
                  {codGloballyActive && (
                    <div>
                    <label className={`flex items-center justify-between p-4 border transition-colors ${!codAllowed ? 'opacity-50 cursor-not-allowed border-border' : payment === 'cod' ? 'border-foreground cursor-pointer' : 'border-border hover:border-muted-foreground cursor-pointer'}`}>
                    ...
                    {!codAllowed && (
                      <p className="text-xs text-muted-foreground mt-1 px-1">{codRule?.note || 'Cash on Delivery is not available for your selected area.'}</p>
                    )}
                    {payment === 'cod' && codAllowed && advanceRequired && (
                      <p className="text-xs text-muted-foreground mt-1 px-1">An advance payment of ৳{advanceForCod.toLocaleString()} is required for your area. You'll be redirected to pay it securely; the rest is collected on delivery.</p>
                    )}
                    </div>
                  )}

                  {/* Automated online gateways (SSLCommerz / bKash / Nagad) */}
                  {automatedMethods.map(m => (
                    <label
                      key={m.gateway}
                      className={`flex items-center justify-between p-4 border cursor-pointer transition-colors ${payment === 'automated' && automatedGateway === m.gateway ? 'border-foreground' : 'border-border hover:border-muted-foreground'}`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${payment === 'automated' && automatedGateway === m.gateway ? 'border-foreground' : 'border-muted-foreground'}`}>
                          {payment === 'automated' && automatedGateway === m.gateway && <div className="w-2 h-2 rounded-full bg-foreground" />}
                        </div>
                        <div>
                          <p className="text-sm">{m.display_name}</p>
                          <p className="text-xs text-muted-foreground">{m.instructions || 'Pay securely online'}{m.mode === 'sandbox' ? ' (Sandbox)' : ''}</p>
                        </div>
                      </div>
                      <input
                        type="radio"
                        name="payment"
                        value={`automated-${m.gateway}`}
                        checked={payment === 'automated' && automatedGateway === m.gateway}
                        onChange={() => {
                          setPayment('automated');
                          setAutomatedGateway(m.gateway);
                          setOnlineProvider(null);
                          setForm(f => ({ ...f, senderNumber: '', transactionId: '' }));
                        }}
                        className="hidden"
                  ...
                  {/* Manual payment gateways (bKash/Nagad/Rocket/Upay/Bank Transfer) — DB-driven */}
                  {manualMethods.map(m => {
                    const r = getManualReceiver(m.manual_config);
                    const selected = payment === 'manual' && selectedManualGateway === m.gateway;
                    return (
                      <div key={m.gateway} className={`border transition-colors ${selected ? 'border-foreground' : 'border-border hover:border-muted-foreground'}`}>
                        <label className="flex items-center justify-between p-4 cursor-pointer">
                          <div className="flex items-center gap-3">
                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${selected ? 'border-foreground' : 'border-muted-foreground'}`}>
                              {selected && <div className="w-2 h-2 rounded-full bg-foreground" />}
                            </div>
                            <div className="flex items-center gap-2">
                              {m.icon ? <img src={m.icon} alt="" className="h-6 w-auto object-contain" /> : null}
                              <div>
                                <p className="text-sm">{m.display_name}</p>
                                <p className="text-xs text-muted-foreground">{m.instructions || 'Send money and submit your transaction details'}</p>
                              </div>
                            </div>
                          </div>
                          <input
                            type="radio"
                            name="payment"
                            value={`manual-${m.gateway}`}
                            checked={selected}
                            onChange={() => {
                              setPayment('manual');
                              setSelectedManualGateway(m.gateway);
                              setAutomatedGateway(null);
                              setOnlineProvider(null);
                              setProofFile(null);
                              setForm(f => ({ ...f, senderNumber: '', transactionId: '' }));
                        ...
                        {selected && (
                          <div className="px-4 pb-4">
                            <div className="p-4 bg-secondary/30 border border-border space-y-4">
                                ...
                                <div className="flex items-center justify-between gap-3 p-3 bg-background border border-border rounded">
                                  <div>
                                    <span className="text-xs text-muted-foreground">
                                      {r.accountType || 'Send to'}{r.accountName ? ` — ${r.accountName}` : ''}
                                    </span>
                                    <p className="font-semibold tracking-wider">{r.number}</p>
                                  </div>
                                  <button
                                    type="button"
                                    onClick={() => handleCopy('number', r.number)}
                              ...
                              <div>
                                <label className="text-xs text-muted-foreground tracking-wider uppercase block mb-2">
                                  Transaction ID {r.requireTxnId && <span className="text-destructive">*</span>}
                                </label>
                                <input
                              ...
                              <div>
                                <label className="text-xs text-muted-foreground tracking-wider uppercase block mb-2">
                                  Payment Screenshot {r.requireScreenshot && <span className="text-destructive">*</span>}
                                </label>
                                <label className="flex items-center gap-2 h-10 px-3 border border-input bg-background rounded-md cursor-pointer hover:bg-accent hover:text-accent-foreground transition-colors text-sm">
                                  <Upload className="h-4 w-4" />
                                  <span className="truncate">{proofFile ? proofFile.name : 'Upload screenshot'}</span>
                                  <input
                                    type="file"
                                    accept="image/*"
                  ...
                  {/* Online Payment (legacy manual bKash/Nagad via checkout_payment_settings) —
                      shown only when no DB-driven manual gateways are configured. */}
            ...
            <div className="lg:sticky lg:top-40 h-fit">
              <div className="bg-card border border-border rounded-2xl p-6 sm:p-7 shadow-sm">
                ...
                <p className="text-sm text-muted-foreground mt-1 mb-5">EIDLIP</p>
                ...
                <div className="flex items-center gap-4 mt-5">
                  <button
                    type="button"
                    onClick={() => navigate('/')}
                    className="flex items-center gap-2 text-sm font-medium text-foreground hover:text-foreground/60 transition-colors"
                  >
                ...
                >
                  {createOrder.isPending || redirecting
                    ? 'Processing...'
                    : (payment === 'automated' || (payment === 'cod' && advanceRequired) ? 'Continue to Payment' : 'Place order')}
                </button>
              </div>
            </div>
          </form>
      ...
      {showConfirmedPopup && (
        <OrderConfirmedPopup
          image={pendingVerification ? '' : orderConfirmedImage}
          title={pendingVerification ? 'Order Submitted for Verification' : orderConfirmedTitle}
          message={pendingVerification
            ? 'আপনার অর্ডার গ্রহণ করা হয়েছে এবং পেমেন্ট যাচাইয়ের জন্য অপেক্ষা করছে। আমাদের টিম যাচাই সম্পন্ন করার পর অর্ডার কনফার্ম করবে।'
            : orderConfirmedMessage}
          buttonLabel={pendingVerification ? 'Close' : orderConfirmedButton}
          paymentMethod={pendingVerification ? confirmedMethodName : undefined}
          statusLabel={pendingVerification ? 'Status: Payment Verification Pending' : undefined}
          onTrack={pendingVerification ? () => { setShowConfirmedPopup(false); navigate('/profile'); } : undefined}
