...
const Wishlist = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO title="Your Wishlist" description="Your saved favorites at EIDLIP." path="/wishlist" noIndex />
      <FabHeader /><CartDrawer />
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-10 pb-20">
        <h1 className="luxury-heading text-3xl tracking-[0.15em] text-center mb-10">Wishlist</h1>
        {items.length === 0 ? (
                ...
                <Link to={`/product/${product.id}`}>
                  <div className="relative aspect-[3/4] overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                      <button onClick={(e) => { e.preventDefault(); const card = (e.currentTarget as HTMLElement).closest('.group'); const sizes = Array.isArray(product.sizes) ? product.sizes : []; const colors = Array.isArray(product.colors) ? product.colors : []; flyToCart(card?.querySelector('img') || e.currentTarget, getProductImage(product.image_url, 400)); addItem(product, sizes[1] || sizes[0] || '', colors[0]?.name || 'Default'); }} className="w-full luxury-button-primary py-3 text-[10px] flex items-center justify-center gap-2">
