...
async function verifyAdmin(req: Request): Promise<{ authorized: boolean; error?: string }> {
  ...
  // Use anon client + getClaims() — works with the new signing-keys system
  const authClient = createClient(supabaseUrl, supabaseAnonKey);
  let userId: string | undefined;
  try {
    const { data, error } = await (authClient.auth as any).getClaims(token);
    if (!error && data?.claims?.sub) userId = data.claims.sub;
  } catch { /* fallback below */ }

  if (!userId) {
    // Fallback to getUser for older JWTs
