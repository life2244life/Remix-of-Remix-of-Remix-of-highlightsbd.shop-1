...
const FROM_EMAIL = `${SITE_NAME} <orders@eidlip.shop>`
const REPLY_TO = 'support@eidlip.shop'
...
function generatePlainText(props: Record<string, unknown>): string {
  ...
  return `EIDLIP - Order Confirmation
...
Thank you for shopping with EIDLIP.`
}
...
function generateAdminPlainText(props: Record<string, unknown>): string {
  ...
  return `New Order Received - EIDLIP
...
If you have any questions, reply to this email or contact us at support@eidlip.shop
    ...
    if (to) {
      ...
      const customerRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `Your EIDLIP order #${shortId}`,
