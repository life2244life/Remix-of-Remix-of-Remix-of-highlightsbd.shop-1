-- Fix #2: Persist UTM attribution snapshot permanently onto orders.

-- 1) Add attribution columns (nullable -> existing orders stay compatible).
...
-- 2) Update create_order RPC to persist the attribution snapshot server-side.
--    Values are read from _order->'attribution', trimmed, length-capped, and
--    written inside the SECURITY DEFINER function (browser never inserts directly).
